...
- expression：根据表达式生成。适用整数类型、浮点数类型 float、double 和字符类型。
  - formula (字符串，必需)：表示生成数据的表达式内容，表达式语法采用 lua 语言，内置变量 `_i` 表示调用索引，从 `0` 开始，如："2 + math.sin(_i/10)"。
    ...
    ```lua
    (math.sin(_i / 7) * math.cos(_i / 13) + 0.5 * (math.random(80, 120) / 100)) * (((_i - 1) % 50 < 25) and (1 + 0.3 * math.sin(_i / 3)) or 0.7) + 10 * (math.floor(_i / 100) % 2)
    ```
    ...
    | C    | `(((_i - 1) % 50 < 25) and (1 + 0.3 * math.sin(_i / 3)) or 0.7)`   | 动态增益调制   | 每 50 次调用切换一次增益（前 25 次高增益，后 25 次低）            |
