...
void CreateDatabaseAction::execute() {
    ...
    try {
        ...
        if (config_.database_info.drop_if_exists) {
            auto drop_result = formatter->format(config_, true);
            connector_->execute(std::get<std::string>(drop_result));
        }

        auto create_result = formatter->format(config_, false);
        connector_->execute(std::get<std::string>(create_result));
